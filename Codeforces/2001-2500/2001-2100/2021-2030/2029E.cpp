...
struct NT {
    ...
    NT(int x) {
        ...
        for (int i = 2; i <= x; i++) if (!sieve[i]) {
            primes.pb(i);
            for (int j = 2; i * j <= x; j++) if (!sieve[i * j]) sieve[i * j] = i;
        }
...
void solve() {
    ...
    for (int i = 1; i <= N; i++) if (nt.sieve[arr[i]]) {
        int v = arr[i] - nt.sieve[arr[i]];
        // cout << v << " ";
        if (v <= 2 * prime && (v % prime) && (arr[i] % prime)) ans = 0;
    }
    // cout << endl;

    if (ans) cout << (prime ? prime : 2) << endl;
    else cout << -1 << endl;
