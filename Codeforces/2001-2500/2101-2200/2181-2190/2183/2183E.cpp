...
void solve() {
    ...
    if ((arr[1] > 0 && arr[1] != 1) || (arr[2] > 0 && arr[2] != 2)) {
        cout << 0 << endl;
        reset_tc();
        return;
    }
    ...
    for (int i = 2; i < N; i++) {
        for (int j = 1; j <= M; j++) {
            for (int div : nt.divisors[j]) if (j + div <= M) {
                addeq(dp[i + 1][j + div], dp[i][j]);
                // if (i == 4 && j + div == 9) cout << "come from " << j << endl;
            }
        }
        if (arr[i + 1] > 0) {
            for (int j = 1; j <= M; j++) if (j != arr[i + 1]) dp[i + 1][j] = 0;
        }
    }
