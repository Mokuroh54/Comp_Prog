...
void solve() {
    ...
    for (int i = 1; i <= N; i++) {
        ...
        if (c == 'R' || c == 'L') {
            if (c == 'R') idx++;
            if (c == 'L') idx = max(idx - 1, 1);
        }
        else {
            if (cur[idx] == '(') {
                if (c == ')') st.update(idx, V, -2, 1, 1, V);
                else if (c != '(') st.update(idx, V, -1, 1, 1, V);
            }
            else if (cur[idx] == ')') {
                if (c == '(') st.update(idx, V, 2, 1, 1, V);
                else if (c != ')') st.update(idx, V, 1, 1, 1, V);
        ...
        // cout << idx << endl;
        // for (int i = 1; i <= 6; i++) cout << st.query(i, i, 1, 1, V).sum << " ";
        // cout << endl;

        ST::Node n = st.query(1, V, 1, 1, V);

        if (n.minv < 0 || st.query(V, V, 1, 1, V).sum) cout << -1 << " ";
        else cout << n.maxv << " ";
    }
    cout << endl;
