...
void reset_tc() {
    for (int i = 1; i <= N; i++) cnt[i] = guys[i] = alive[i] = counted[i] = 0;
}
...
void solve() {
    ...
    for (int i = 1; i <= N; i++) guys[i] = guys[i] * (guys[i] - 1) / 2;

    for (int i = 1; i <= N; i++) if (cnt[i]) {
        for (int j = i; j <= N; j += i) alive[j] = 1;
    }

    ll ans = 1ll * N * (N - 1) / 2;
    for (int i = N; i >= 1; i--) {
        for (int j = i; j <= N; j += i) guys[i] -= counted[j];

        counted[i] += guys[i];
        if (alive[i]) {
            // cout << "extracting " << guys[i] << " from " << i << endl;
            ans -= guys[i];
        }
    }
